...
> **Status note (August 2026).** The bullet above — "new concepts emerge when posts cluster in ways that don't match anything yet" — went unimplemented for months. Mechanical discovery created concepts only from shared URLs/mentions, and the semantic pass could only match *existing* centroids, so 51% of live posts held no concept edge. `discover_orphan_clusters()` (pipeline step 3.55) now covers the clustering half of that intent, and the blinded latent pass covers the other half. See CLAUDE.md for tuned constants and the mean-centering rationale.
>
> **On when the gate actually opened.** Nothing records the gate ratio over time, so a reading taken today looks like a state that just arrived. It didn't: `enrichment_last_attempt_at` shows the recoverable backlog was drained in a burst over **2026-07-14 → 07-17** (356 posts in four days), with everything after that a trickle of newly-arrived posts. The gate has been open since roughly **2026-07-17** — about four weeks before the latent pass was finally written. The delay was not the gate; it was that nobody had written the pass. `pipeline.gate_history` now snapshots the ratio each run so this is answerable from data next time.
