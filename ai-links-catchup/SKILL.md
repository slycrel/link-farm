...
4. Extract structured segments via `javascript_tool`: enumerate `<article>` elements; for each, capture `(text, handle, urls)`. The first article is type `op`; subsequent articles whose handle matches the page handle are `self_reply`; others are `quote`. Detect external links (github / arxiv / huggingface / blog hosts) as their own `external_link` segments.
   ...
   **You must also capture the quoted-post and link-card block, or roughly half these posts capture as meaningless.** A large share of this corpus is endorsement posts — "the best agent infrastructure article I've read recently", "you must read!", "banger paper" — where the tweet text alone carries no information and the entire substance lives in the quoted article or link card. X renders those as `div[role="link"]` inside the article (and `[data-testid="card.wrapper"]` for bare link cards), *not* as sibling `<article>` elements, so an extractor that only walks `<article>` silently drops them. Capture both:

   ```js
   const quoted = [...document.querySelectorAll('article div[role="link"]')]
       .map(x => x.innerText.replace(/\n+/g, ' | ').slice(0, 420))
       .filter(Boolean).slice(0, 2);
   const card = document.querySelector('[data-testid="card.wrapper"]');
   ```

   The quoted block usually yields the linked article's title plus its opening paragraph — enough to write a real summary. Persist it as a `quote` (or `external_link`) segment.

5. On timeout/empty extraction, retry once after 3s.
6. **Dead detection — require a positive marker. Empty extraction is NOT death.**

   An X **Article** (long-form post) has no `[data-testid="tweetText"]` element at all: the body lives in the page, not in a tweet-text node. A live, perfectly healthy Article therefore extracts as empty text, and a dead-check that treats "no text" as a tombstone will permanently kill it. `dead` is never retried, so this failure mode is unrecoverable and silent — it is the worst bug this skill can have. It has already happened once (`@KSimback`'s "Hermes Agent Memory Guidebook", 84.8K views, was flagged dead by an empty-extraction heuristic and was fully alive).

   Before recording `dead`, both must hold:
   - extraction produced nothing usable, **and**
   - `document.body.innerText` contains an explicit tombstone marker: "post is unavailable" / "account suspended" / "page doesn't exist" / "this Tweet was deleted" / "this account doesn't exist".

   And scope the marker test to the main column. Those strings also appear in "Discover more" rails, quoted-post placeholders, and replies further down the page, so testing the whole body produces false positives on live posts. Confirm the page also lacks live-post signals — a view count, a timestamp, an author name — before concluding it's gone.
   ...
   record_dead(post_id, reason=detected_marker,
               ...
               with_lock=False)
   ```
   Move on. `dead` posts are permanent and never retried. **When in doubt, `record_partial` instead** — a partial is retried next run and costs one extra scrape; a wrong `dead` is forever.
...
- **Quote tweets** — `op` segment for the quoting tweet plus a `quote` segment for the quoted content. **The quoted post is a `div[role="link"]` nested inside the quoting `<article>`, not a sibling `<article>`** — walking `<article>` elements alone misses it entirely. Since endorsement posts ("must read", "best article I've seen") carry all their meaning in the quoted block, dropping it reduces the capture to noise. See step 4 of the enrichment loop.
- **X articles** — long-form. **These have no `[data-testid="tweetText"]` node**, so a text-only extractor returns empty on a perfectly live post — never infer death from that (see step 6). Capture the article body from the page as the `op` segment text (extend the truncation limit if needed) plus any commentary from the author's introducing tweet.
