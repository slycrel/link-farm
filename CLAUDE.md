...
- **db/filing.py** — Abstain-aware filing classifier (Sept 2026). A *review instrument*, not an auto-filer: it scores each post against a bounded candidate set on **mean-centered** vectors and **abstains** when the top-two margin is thin, instead of always naming a winner the way `assign_primaries()` must. Motivated by a measurement — on 435 contested posts, **86% of raw filing decisions land inside a 0.01 cosine margin**, i.e. most primary homes are coin flips with a confident number attached. Centering lifts the median margin ~16x (0.0014 → 0.0222). **Bounding matters as much as centering:** centered-but-unbounded scoring *increases* homes landing on tiny `url:`/`mention:` groupings (raw 21 → 29, argmax over all 577 edged posts), because a 2-member centroid is sharp and idiosyncratic; bounding to conceptual concepts drops it to 4. All thresholds are **centered** cosines, not comparable to the raw `SEMANTIC_*` constants. `ABSTAIN_MARGIN` defaults to 0.03, the swept point where the classifier proposes *zero* reassignments and becomes purely additive. An abstention is a **no-op, never an un-homing**. Contains no dismissal path (enforced by test). CLI: `python3 -m db.filing {report,review,calibrate,apply}`; `apply` is dry-run unless `--confirm`. Tests: `db/test_filing.py` (18). Surface: the `ai-links-filing` skill.

  **Calibration is label-free, and it has to be — there is no usable ground truth.** Of 577 primary homes, **139 are single-candidate** (forced, so they carry no information about discrimination) and **373 of the 435 contested were decided on a sub-0.01 raw margin (86%)**, leaving roughly **62** that represent a defensible decision. Scoring the classifier against the existing homes would therefore measure agreement with noise, and the accuracy would *improve* as it got better at reproducing coin flips. Do not build an eval on them. `calibrate` instead bootstraps each candidate concept's membership (seeded, reproducible) and asks whether the same home still wins — a decision that doesn't survive resampling was an artifact of which posts happened to be attached. Measured 2026-09-25 over 436 contested posts, 15 resamples:
  ...
  Monotonic, which is what justifies the margin-based design. At an 80% target `calibrate` recommends **0.03** — independently reproducing the default that was chosen for an unrelated reason (zero reassignments), which is the closest thing to external validation available here. **But quote it honestly:** 0.03 clears the target *on average* (83.3% over 186 retained), while the 0.03–0.05 band inside it is only 61.7% — the mean is carried by the high-margin tail. `--target 0.9` gives 0.05, retaining 132 at 92.1%. Re-run after any large merge/split/re-home, since those reshape the centroids the threshold was derived from.

  **The untried upgrade is still a trained head**, and the blocker is labels, not machinery: a head fitted to today's homes would learn the coin flips. That needs a hand-labelled gold set, sampled **stratified across margin bands** — a random sample is dominated by easy cases and would flatter any classifier.
...
Two honest limits: it inherits bge-small's geometry and learns nothing about how Jeremy actually files (a trained head over the 577 existing home decisions is the untried upgrade), and it **cannot fix a magnet, only reveal one** — a drifted centroid poisons every post scored against it, so run the magnet detector first.
...
Regression tests: **`db/test_roles.py`** (33 tests as of 2026-09-25). It exists because every one of those four is a silent failure if it regresses.
...
**The #74 incident, 2026-09-25 — the sharpest version of this failure yet, because it became self-defeating.** #74 *System One models — bounded decisions as a primitive* was hand-seeded 2026-09-24 23:28 with 9 Jev posts, precisely because the semantic layer was scattering that conversation across five unrelated homes. It graduated the same day and within 48 hours held **328 evidence edges, of which only 10 concerned Jev / CLM / RLCD.** (Edge accounting: `concept_observations` shows 93 promoted on 09-24 and 245 on 09-25, but the latter includes the hand-curated CLM edge added *after* the rollback — so peak evidence is 318 rolled back + 11 held now − 1 curated = 328.) The rest reached back to 2025-01-04: agent memory, agent factories, eval pipelines, harness recipes. The diagnostic that matters:
...
**Known pre-existing issue: concept centroids are barely discriminative.** **191 of 1,128 centroid pairs sit above 0.99 cosine (16.9%, re-measured 2026-09-25)** — several large concepts have effectively converged on the same direction. This predates the role work (141 of 378 pairs were already >0.99 on 2026-08-24, i.e. 37% then vs 16.9% now; the ratio keeps falling because the denominator grows faster than the offending pairs do) and is the raw-cosine problem the orphan-clustering note describes: on this corpus everything is "AI stuff", so un-centered centroid similarity is dominated by that shared direction.
...
## Collection Stats (as of September 25, 2026)
...
- **903 total posts** in SQLite (live count). Date range: June 11, 2024 – September 25, 2026.
- Enrichment status: **830 `ok`** (current `ENRICHMENT_VERSION=1`), 73 `dead` (permanent floor — deleted/suspended/login-walled). **Zero `partial` / `failed` / `unattempted` / `legacy-ok` — the enrichment queue has been completely empty since 2026-08-26**, which is why the backfill task is dormant (see the Automation section).
...
- Top topics: agent-design (541), dev-practices (392), research (243), skills-mcp (203), claude-code (198), management (140), questionable (138), industry (135), prompting (135), general (100), `adjacent` (45), `biohacking` (8), `solo-operator` (8).
...
- Priority breakdown: near-term (610), long-term (179), now (114).
- Audiences: me (895), dev-team (645), leadership (169), team (5).
- Concept graph (2026-09-25): **55 active + 1 provisional** (10 archived, 8 merged-into). 8,064 edges — 5,469 `evidence` + 2,593 `weak` + 2 `counter-example`. **Quote the primary-home count, not the evidence count and not the total** — see the set-identity finding above; evidence overstates a concept by 10–60x and the totals include the generous weak layer. **577 primary homes across 34 concepts**, 0 pending observations. Largest homes: #41 *Claude Code setup & usage* (67), *vector / hybrid databases as agent-memory infrastructure* (57), #19 *agent harness engineering* (54).
- **The evidence count fell 5,786 → 5,469 on 2026-09-25** without anything being dismissed. That is the #74 rollback: 318 over-attached edges demoted `evidence → weak`, offset by 1 hand-curated edge added after (5,786 − 318 + 1 = 5,469), which is also why `weak` rose. If you are comparing edge counts across that date, this is the reason — it is not data loss, and `_rollback_74_20260925` holds the exact reversible set.
- **Two concepts now carry `[no-centroid-scoring]`**: #65 *thinking tools & problem-solving method* (2026-08-26) and #74 *System One models* (2026-09-25). As of 2026-09-25 the marker is applied **by default** to reader-seeded concepts, so this count should stop being a measure of vigilance and start being a measure of how many hand-made concepts exist.
- **The magnet detector currently returns 11 hits** (2026-09-25) — #48, #55, #47, #45, #51, #52, #3, #22, #40, #39, #4, all with canonical edges > 40 and > 10× their primary-home count. These are the same concepts the set-identity finding describes (82–94% Jaccard member overlap, re-measured 2026-09-25; the 84–93% quoted elsewhere is the 2026-09-15 measurement) plus four more. The new default prevents *new* magnets; it does nothing for these. Remediating them is redesign work, deliberately deferred (Jeremy, 2026-09-25) — the corpus is in hand-tooling territory and the home layer is unaffected, since primaries remain a clean partition.
- **21 of the 55 active concepts are home to zero posts** (2026-09-25; was 21 of 54 on 2026-09-17) — they exist only as secondary tags, and many are `url:` groupings. This is the empty-shell problem in a second form: the regression query in the bullet below checks for concepts with *no edges*, which these pass. The sharper query is `SELECT id,name FROM concepts c WHERE status='active' AND NOT EXISTS(SELECT 1 FROM post_concepts pc WHERE pc.concept_id=c.id AND pc.is_primary=1)`.
...
- Orphans (no canonical home): **251 live posts** (2026-09-25), of which only 53 have *no edge at all* — **198 have edges that are all `weak`**. The count moved by the #74 rollback in both directions: three posts that had been homed at #74 only because the magnet absorbed them became honestly unhomed, while the new CLM post gained a home. Prefer honestly-orphaned to wrongly-filed. **56 sit at a best-candidate cosine of 0.80–0.81**, just under the 0.82 floor (re-measured 2026-09-25; the 63 quoted in earlier vintages was carried forward from 2026-09-17 without re-measuring — re-measure this one, it drifts). The topic bias the role work fixed has stayed fixed (`adjacent` is 1.06x baseline, was 1.6x). Known narrow gap: a post whose *only* edges came from `revive_dismissed.py` can never be homed, since that script deliberately attaches recovered history as `weak` regardless of cosine — this strands exactly 3 posts whose best revived edge cleared 0.82 (Garry Tan 0.824/#19, Phil Chen 0.825/#5, Avid 0.827/#2). Not a bug; a policy edge case.
...
- Observation provenance (2026-09-25): semantic 7,895 promoted / 6,017 dismissed, mechanical 135 / 19, cluster 58 promoted, latent 16 promoted, curated 26 promoted. **The dismissed figure has not moved since 2026-08-26 and must not** — see the no-discard check below.
...
```
- 39 posts carry a subject `flag:` in `notes`.
...
├── decisions/                   # Dated decision records (the *why*)
│   └── 2026-09-25-filing-and-the-74-magnet.md
