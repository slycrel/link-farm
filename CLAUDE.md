...
## ⏳ Open work queue (resume here — as of 2026-08-24)

**Both known-bad data populations are now closed.** The queue is empty apart from one single-post capture retry (item 1a). Everything here is *detected by query*, so nothing depends on remembering a list.

**1. Legacy description-blob content — ✅ CLOSED 2026-08-24.** All 236 are done; the detector below returns **1**, and that one is the `partial` retry in item 1a, not a blob awaiting rewrite. A capture-logic vintage from roughly March–May 2026 wrote an editorial *description* into the `op` segment instead of the post's actual text, shaped `Author @handle. <description>. 7.4M views. topic1, topic2.` All were marked `v1`/`ok`, so they were invisible to `pending_enrichment_ids()`, and all were embedded — meaning a large slice of the semantic layer was voting on concept assignment using paraphrase rather than source text. The detector, kept for regression use:
...
**1a. One post pending capture retry — `2055064462844219603` (Sam Hogan, X Article, 23.9K views).** Only unresolved item in the queue. Three scrape attempts returned `[BLOCKED: Cookie/query string data]` in place of page text — the Chrome extension's own content guard redacting the payload, *not* a dead post: the page renders 14.2K chars into `main`, exposes 2 article nodes and a live view count, and carries no tombstone marker. Recorded via `record_partial` (attempts=2) with that diagnosis in `enrichment_last_error`, so `ai-links-backfill` will retry it unattended. Its `op` segment still holds the old blob, which is why the detector above returns 1. Sibling post `2056484487891243355` (Amar Singh) cites the same repo — `github.com/context-labs/HALO` — so the content is partly recoverable from that side if the retry keeps failing. **Do not mark it `dead`.**
...
## Collection Stats (as of August 24, 2026)
...
- **816 total posts** in SQLite (live count). Date range: June 11, 2024 – August 21, 2026.
- Enrichment status: 742 `ok` (current `ENRICHMENT_VERSION=1`), 73 `dead` (permanent floor — deleted/suspended/login-walled), 1 `partial` (the item-1a capture retry). **Zero `failed` / `unattempted` / `legacy-ok`.** `ai-links-backfill` has exactly one post to chew on and otherwise no-ops.
- **Latent gate ratio: 0.13% — the gate is OPEN** (open since 2026-08-14; see `gate_history`). Latent discovery is available on demand, but only inside a skill session — it is deliberately not in `post_enrichment_pipeline` because an unattended run has no reader.
- Top topics: agent-design (478), dev-practices (342), research (208), claude-code (188), skills-mcp (185), management (127), prompting (122), industry (117), questionable (116), general (100), `adjacent` (31), `solo-operator` (6), `biohacking` (5).
...
- Priority breakdown: near-term (528), long-term (178), now (110).
- Audiences: me (808), dev-team (572), leadership (152), team (5).
- Concept graph: 49 active concepts (10 archived, 4 merged-into), 4,154 edges, 498 primary homes, 0 pending observations. 318 posts still carry no concept edge (245 of them embedded and thus eligible for orphan clustering; the other 73 are `dead` and never embedded).
- **Caveat on the active-concept count: 20 of the 49 are empty `url:` / `mention:` shells with zero edges.** They are created by mechanical discovery, then stripped of their observations by `auto_curate()` as low-signal per-URL/per-person groupings whose posts are already conceptually covered — leaving the concept row behind with nothing attached. So the real conceptual vocabulary is **29 concepts**, and any report quoting "49 active" is overstating it. Harmless but untidy; archiving them is a structural call for `ai-links-curate`, not something a catch-up run should do unasked. Query: `SELECT id,name FROM concepts c WHERE status='active' AND NOT EXISTS(SELECT 1 FROM post_concepts pc WHERE pc.concept_id=c.id)`.
- Observation provenance: semantic 4,066 promoted / 2,560 dismissed, mechanical 66 / 70, cluster 58 promoted, latent 16 promoted.
- 32 posts carry a subject `flag:` in `notes`.
