...
`db/concepts.split_candidates()` flags active concepts at/above `SPLIT_CANDIDATE_MIN_POSTS` (default 70) as candidates worth vetting for sub-categorization. Advisory only — surfaced in the pipeline summary (`⚑ split-review candidates: …`), never an auto-split. Overlapping/broad concepts are acceptable; the trigger just keeps a big bucket from quietly accumulating unnoticed.
